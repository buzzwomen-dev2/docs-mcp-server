...
Psycopg can process queries expressed as [template strings](https://docs.python.org/3.14/whatsnew/3.14.html#pep-750-template-string-literals) defined in
[**PEP 750**](https://peps.python.org/pep-0750/) and implemented for the first time in Python 3.14.

Template strings are similar to [f-strings](https://docs.python.org/3/tutorial/inputoutput.html#tut-f-strings): they are string literals
...
executing it directly, you can use the [`sql.as_string()`](../api/sql.md#psycopg.sql.as_string) or [`as_bytes()`](../api/sql.md#psycopg.sql.as_bytes)
functions:
...
the [identifier syntax](https://www.postgresql.org/docs/current/sql-syntax-lexical.html#SQL-SYNTAX-IDENTIFIERS) before continuing with the normal processing (which
...
format specifiers specifying [how to pass a parameter to the server](params.md#binary-data) and a few format specifiers specifying how to compose query
parts on the client, in a way similar to what can be obtained using the
[`psycopg.sql`](../api/sql.md#module-psycopg.sql) objects.
...
- `i`: the parameter is an [identifier](https://www.postgresql.org/docs/current/sql-syntax-lexical.html#SQL-SYNTAX-IDENTIFIERS), for example a table or column name.
  The parameter must be a string or a [`sql.Identifier`](../api/sql.md#psycopg.sql.Identifier) instance.
- `l`: the parameter is a literal value, which will be merged to the
  query on the client. This allows to parametrize statements that [don’t
  support parametrization in PostgreSQL](from_pg2.md#server-side-binding).
- `q`: the parameter is a snippet of statement to be included verbatim in
  ...
  [`sql.SQL`](../api/sql.md#psycopg.sql.SQL)/[`Composed`](../api/sql.md#psycopg.sql.Composed) instance.

<a id="tstring-template-notify"></a>

## Example: NOTIFY

The [NOTIFY](https://www.postgresql.org/docs/current/sql-notify.html) command takes a *channel* parameter (an identifier, so it must be
...
The `NOTIFY` command cannot be parametrized by PostgreSQL, so it must be
...
<a id="tstring-template-nested"></a>
...
In this example we have used the [`sql.SQL.join()`](../api/sql.md#psycopg.sql.SQL.join) method overload that takes a
