...
In short, if you use one of the [supported systems](#supported-systems),
run:
...
and you should be [ready to start](usage.md#module-usage). Read further for
...
- PostgreSQL: from version 10 to 18
  - **Note:** PostgreSQL [currently supported release](https://www.postgresql.org/support/versioning/) are actively tested
    in the CI. Out-of-support releases are supported on a best-effort basis.
- OS: Linux, macOS, Windows
- Pip: 20.3 or newer

The tests to verify the supported systems run in [Github workflows](https://github.com/psycopg/psycopg/actions):
...
| Installation Option                                                      | Command                       | Description                                                                     | Perfor­mance   | Needs Local libpq?   | Needs Build Tools?   |
|--------------------------------------------------------------------------|-------------------------------|---------------------------------------------------------------------------------|----------------|----------------------|----------------------|
| [Binary installation](#binary-installation) (recommended for most users) | `pip install psycopg[binary]` | Precompiled C extensions, packaged with client lbiraries.                       | 🐇 Fast         | ❌ No                 | ❌ No                 |
| [Local installation](#local-installation)                                | `pip install psycopg[c]`      | Builds C extensions from source.<br/>Requires build tools and client libraries. | 🐇 Fast         | ✅ Yes                | ✅ Yes                |
| [Pure Python installation](#pure-python-installation)                    | `pip install psycopg`         | Pure Python implementation only. Requires client libraries.                     | 🐢 Slow         | ✅ Yes                | ❌ No                 |

<a id="binary-installation"></a>
...
#### SEE ALSO
Did Psycopg 3 install ok? Great! You can now move on to the [basic
module usage](usage.md#module-usage) to learn how it works.
...
[pq module implementations](../api/pq.md#pq-impl).

If your platform is not supported, or if the libpq packaged is not suitable,
you should proceed to a [local installation](#local-installation) or a
[pure Python installation](#pure-python-installation).
...
- binary packages for a new version of Python are made available once
  ...
  [psycopg-binary PyPI files](https://pypi.org/project/psycopg-binary/#files) to verify whether your platform is
  supported;
- the libpq version included in the binary packages depends on the version
  available on the runners. You can use the [`psycopg.pq.version()`](../api/pq.md#psycopg.pq.version)
  function and [`__build_version__`](../api/pq.md#psycopg.pq.__build_version__) constant to infer the
  features available.
...
- Starting from Psycopg 3.1.20, ARM64 macOS binary packages (i.e. for
  ...
  Please upgrade your OS to at least 14.0 or use a [local](#local-installation) or a [Python](#pure-python-installation)
  installation.
...
try this and use the [Binary installation](#binary-installation) instead.
...
<a id="pure-python-installation"></a>
...
[Binary installation](#binary-installation) or [Local installation](#local-installation). It is likely
...
dynamically via [`ctypes`](https://docs.python.org/3/library/ctypes.html#module-ctypes)); for example on Debian systems you will probably
need:
...
[Binary installation](#binary-installation), which packages its own copy of the `libpq`.

#### NOTE
The `libpq` is the client library also used by **psql**, the
PostgreSQL command line client, to connect to the database.  On most
systems, installing **psql** will install the `libpq` too as a
dependency.

<a id="pool-installation"></a>

## Installing the connection pool

The [Psycopg connection pools](../advanced/pool.md#connection-pools) are distributed in a
...
install "psycopg[pool]"`, or install the [`psycopg_pool`](../api/pool.md#module-psycopg_pool) package separately,
...
will fail if the implementation specified is not available. See [pq module implementations](../api/pq.md#pq-impl).
