...
more expressive [template string queries](tstrings.md#template-strings).
...
[`Cursor.execute()`](../api/cursors.md#psycopg.Cursor.execute) by using `%s` placeholders in the SQL statement, and
...
parameters are sent to the server separately: see [Server-side binding](from_pg2.md#server-side-binding)
for details.
...
- The variables placeholder *must always be a* `%s`, even if a different
  ...
  type of the argument: see [Binary parameters and results](#binary-data) if you want to read more:
  ```default
...
- Only query values should be bound via this method: it shouldn’t be used to
  ...
  functionalities provided in the [`psycopg.sql`](../api/sql.md#module-psycopg.sql) module:
  ```default
...
is called [SQL injection](https://en.wikipedia.org/wiki/SQL_injection) and is known to be one of the most widespread forms
of attack on database systems. Before continuing, please print [this page](https://xkcd.com/327/)
as a memo and hang it onto your desk.

Psycopg can [automatically convert Python objects to SQL
values](adapt.md#types-adaptation): using this feature your code will be more robust
...
- You don’t want to manually merge values to a query: [use the
  provided methods](#query-parameters) instead.

The correct way to pass variables in a SQL command is using the second
argument of the [`Cursor.execute()`](../api/cursors.md#psycopg.Cursor.execute) method:
...
expressions in queries. See [Checking literal strings in queries](../advanced/typing.md#literal-string) for details.

#### SEE ALSO
Now that you know how to pass parameters to queries, you can take a look
at [how Psycopg converts data types](adapt.md#types-adaptation).
...
[`TEXT`](../api/pq.md#psycopg.pq.Format.TEXT), always available, and [`BINARY`](../api/pq.md#psycopg.pq.Format.BINARY),
...
placeholder instead of the normal `%s`. [`execute()`](../api/cursors.md#psycopg.Cursor.execute) will fail if a
[`Dumper`](../api/adapt.md#psycopg.adapt.Dumper) for the right data type and format is not available.
...
format. Every type returned by the query should have a [`Loader`](../api/adapt.md#psycopg.adapt.Loader)
configured, otherwise the data will be returned as unparsed `str` (for text
results) or buffer (for binary results).

#### NOTE
The [pg_type](https://www.postgresql.org/docs/current/catalog-pg-type.html) table defines which format is supported for each PostgreSQL
...
can create the cursor using [`Connection.cursor`](../api/connections.md#psycopg.Connection.cursor)`(binary=True)` or execute
the query using [`Cursor.execute`](../api/cursors.md#psycopg.Cursor.execute)`(binary=True)`. A case in which
...
#### WARNING
You cannot execute [multiple statements in the same query](from_pg2.md#multi-statements) when requesting a binary result.
