...
**psql**: by default, any database operation will start a new
transaction. As a consequence, changes made by any cursor of the connection
will not be visible until [`Connection.commit()`](../api/connections.md#psycopg.Connection.commit) is called, and will be
discarded by [`Connection.rollback()`](../api/connections.md#psycopg.Connection.rollback). The following operation on the same
...
One obvious problem after the run above is that, firing up **psql**,
...
use the [connection context](usage.md#with-connection), Psycopg will commit the
...
transactions using an [autocommit transaction](#autocommit) and/or
[transaction contexts](#transaction-context).
...
use an [`autocommit`](../api/connections.md#psycopg.Connection.autocommit) connection.
...
[`rollback()`](../api/connections.md#psycopg.Connection.rollback) if you want to keep on using the same connection.

<a id="autocommit"></a>

## Autocommit transactions

The manual commit requirement can be suspended using [`autocommit`](../api/connections.md#psycopg.Connection.autocommit),
either as connection attribute or as [`connect()`](../api/connections.md#psycopg.Connection.connect)
parameter. This may be required to run operations that cannot be executed
inside a transaction, such as `CREATE DATABASE`, `VACUUM`,
`CALL` on [stored procedures](https://www.postgresql.org/docs/current/xproc.html) using transaction control.
...
autocommit connection too, using an explicit [transaction block](#transaction-context).

<a id="transaction-context"></a>

## Transaction contexts

A more transparent way to make sure that transactions are finalised at the
right time is to use `with` [`Connection.transaction()`](../api/connections.md#psycopg.Connection.transaction) to create a
...
`transaction()` block will only manage [a savepoint sub-transaction](#nested-transactions), leaving the caller to deal with the main transaction,
as explained in [Transactions management](#transactions):
...
<a id="nested-transactions"></a>

### Nested transactions

Transaction blocks can be also nested (internal transaction blocks are
implemented using [SAVEPOINT](https://www.postgresql.org/docs/current/sql-savepoint.html)): an exception raised inside an inner block
...
transaction block, by raising the [`Rollback`](../api/objects.md#psycopg.Rollback) exception. The exception “jumps”
...
Each `Transaction` object exposes a [`status`](../api/objects.md#psycopg.Transaction.status) property allowing
...
The `status` property is a [`psycopg.Transaction.Status`](../api/objects.md#psycopg.psycopg.Transaction.Status) enum value:
...
<a id="transaction-characteristics"></a>

## Transaction characteristics

You can set [transaction parameters](https://www.postgresql.org/docs/current/sql-set-transaction.html) for the transactions that Psycopg
handles. They affect the transactions started implicitly by non-autocommit
transactions and the ones started explicitly by [`Connection.transaction()`](../api/connections.md#psycopg.Connection.transaction) for
both autocommit and non-autocommit transactions.

#### WARNING
Transaction parameters [don’t affect autocommit connections](from_pg2.md#transaction-characteristics-and-autocommit), unless a `transaction()`
...
[default_transaction_isolation](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-DEFAULT-TRANSACTION-ISOLATION)).

In order to set these parameters you can use the connection attributes
[`isolation_level`](../api/connections.md#psycopg.Connection.isolation_level), [`read_only`](../api/connections.md#psycopg.Connection.read_only),
[`deferrable`](../api/connections.md#psycopg.Connection.deferrable). For async connections you must use the equivalent
[`set_isolation_level()`](../api/connections.md#psycopg.AsyncConnection.set_isolation_level) method and similar. The parameters
can only be changed if there isn’t a transaction already active on the
connection.

#### Versionadded
Added in version 3.2: Added methods equivalent to setting the properties (such as
[`set_isolation_level()`](../api/connections.md#psycopg.Connection.set_isolation_level)) on sync connections too.

#### WARNING
Applications running at [`REPEATABLE_READ`](../api/objects.md#psycopg.IsolationLevel.REPEATABLE_READ) or
[`SERIALIZABLE`](../api/objects.md#psycopg.IsolationLevel.SERIALIZABLE) isolation level are exposed to serialization
failures. [In certain concurrent update cases](https://www.postgresql.org/docs/current/transaction-iso.html#XACT-REPEATABLE-READ), PostgreSQL will raise an
...
implementing the [two-phase commit extensions](https://www.python.org/dev/peps/pep-0249/#optional-two-phase-commit-extensions) proposed by the DBAPI.

The DBAPI model of two-phase commit is inspired by the [XA specification](https://publications.opengroup.org/c193),
...
[`Connection.xid()`](../api/connections.md#psycopg.Connection.xid) method. Once you have a transaction id, a distributed
transaction can be started with [`Connection.tpc_begin()`](../api/connections.md#psycopg.Connection.tpc_begin), prepared using
[`tpc_prepare()`](../api/connections.md#psycopg.Connection.tpc_prepare) and completed using [`tpc_commit()`](../api/connections.md#psycopg.Connection.tpc_commit) or
[`tpc_rollback()`](../api/connections.md#psycopg.Connection.tpc_rollback).  Transaction IDs can also be retrieved from the
database using [`tpc_recover()`](../api/connections.md#psycopg.Connection.tpc_recover) and completed using the above
...
[`Xid`](../api/objects.md#psycopg.Xid) objects can represent both XA-style transactions IDs (such as the ones
...
database is the same employed by the [PostgreSQL JDBC driver](https://jdbc.postgresql.org/): this should
...
For further details see the documentation for the [Two-Phase Commit support methods](../api/connections.md#tpc-methods).
