<a id="static-typing"></a>

# Static Typing

Psycopg source code is annotated according to [**PEP 0484**](https://peps.python.org/pep-0484/) type hints and is
checked using the current version of [Mypy](http://mypy-lang.org/) in `--strict` mode.
...
Psycopg [`Connection`](../api/connections.md#psycopg.Connection) and [`Cursor`](../api/cursors.md#psycopg.Cursor) objects are [`Generic`](https://docs.python.org/3/library/typing.html#typing.Generic) objects and
...
constructor or to the [`cursor()`](../api/connections.md#psycopg.Connection.cursor) method.

By default, methods producing records such as [`Cursor.fetchall()`](../api/cursors.md#psycopg.Cursor.fetchall) return
normal tuples of unknown size and content. As such, the [`connect()`](../api/module.md#psycopg.connect) function
returns an object of type `psycopg.Connection[tuple[Any, ...]]` and
[`Connection.cursor()`](../api/connections.md#psycopg.Connection.cursor) returns an object of type `psycopg.Cursor[tuple[Any,
...
of the [`connect()`](../api/connections.md#psycopg.Connection.connect) and the [`cursor()`](../api/connections.md#psycopg.Connection.cursor) method, which
...
[Row factories](rows.md#row-factories) for more details.
...
The `count(*)` will always return a record with a number, even if the
...
Since Psycopg 3.3, cursors are [iterables](https://docs.python.org/3/glossary.html#term-iterable), therefore they support the
[`next`](https://docs.python.org/3/library/functions.html#next) function. A `next(cur)` will behave like `cur.fetchone()`, but it
...
Similarly, in async code, you can use an `await` [`anext`](https://docs.python.org/3/library/functions.html#anext)`(cur)` expression.
...
The [`ConnectionPool`](../api/pool.md#psycopg_pool.ConnectionPool) class and similar are generic on their
`connection_class` argument. The [`connection()`](../api/pool.md#psycopg_pool.ConnectionPool.connection)
method is annotated as returning a connection of that type, and the record
returned will follow the rule as in [Type of rows returned](#row-factory-static).
...
<a id="example-pydantic"></a>

## Example: returning records as Pydantic models

Using [Pydantic](https://pydantic-docs.helpmanual.io/) it is possible to enforce static typing at runtime. Using a
...
<a id="literal-string"></a>

## Checking literal strings in queries

The [`execute()`](../api/cursors.md#psycopg.Cursor.execute) method and similar should only receive a literal
string as input, according to [**PEP 675**](https://peps.python.org/pep-0675/). This means that the query should
...
If you are composing a query dynamically you should use the [`sql.SQL`](../api/sql.md#psycopg.sql.SQL) object
...
doesn’t implement it](https://github.com/python/mypy/issues/12554), [Pyre](https://pyre-check.org/) does, but [doesn’t work with psycopg yet](https://github.com/facebook/pyre-check/issues/636)).
