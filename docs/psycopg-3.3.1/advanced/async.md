...
- [`Connection`](../api/connections.md#psycopg.Connection) objects *are thread-safe*: more than one thread at time can use
  the same connection. Different thread can use the same connection by
  creating different cursors.
- [`Cursor`](../api/cursors.md#psycopg.Cursor) objects *are not thread-safe*, and are not designed to be used by
...
the [max_connections](https://www.postgresql.org/docs/current/runtime-config-connection.html#GUC-MAX-CONNECTIONS) server setting.
...
[connection pool](pool.md#connection-pools).

#### WARNING
*Connections are not process-safe* and cannot be shared across processes,
for instance using the facilities of the [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing) module.
...
Psycopg [`Connection`](../api/connections.md#psycopg.Connection) and [`Cursor`](../api/cursors.md#psycopg.Cursor) have counterparts [`AsyncConnection`](../api/connections.md#psycopg.AsyncConnection) and
[`AsyncCursor`](../api/cursors.md#psycopg.AsyncCursor) supporting an [`asyncio`](https://docs.python.org/3/library/asyncio.html#module-asyncio) interface.
...
An `AsyncConnection` can be used by several [`asyncio.Task`](https://docs.python.org/3/library/asyncio-task.html#asyncio.Task) at the same time.
However, as with threads, all the [`AsyncCursor`](../api/cursors.md#psycopg.AsyncCursor) on the same connection will
share the same session and will have their access to the connection
serialized.

#### Versionchanged
Changed in version 3.1: [`AsyncConnection.connect()`](../api/connections.md#psycopg.AsyncConnection.connect) performs DNS name resolution in a non-blocking
way.

#### WARNING
Before version 3.1, [`AsyncConnection.connect()`](../api/connections.md#psycopg.AsyncConnection.connect) may still block on DNS
name resolution. To avoid that you should [set the hostaddr connection
parameter](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-PARAMKEYWORDS), or use the [`resolve_hostaddr_async()`](../api/dns.md#psycopg._dns.resolve_hostaddr_async) to
do it automatically.

#### WARNING
On Windows, Psycopg is not compatible with the default
[`ProactorEventLoop`](https://docs.python.org/3/library/asyncio-eventloop.html#asyncio.ProactorEventLoop). Please use a different loop, for instance
the [`SelectorEventLoop`](https://docs.python.org/3/library/asyncio-eventloop.html#asyncio.SelectorEventLoop). See [asyncio documentation](https://docs.python.org/3.14/library/asyncio-eventloop.html#asyncio.SelectorEventLoop) for details.
...
As seen in [the basic usage](../basic/usage.md#usage), connections and cursors can act as
...
not quite. Please note that [`connect()`](../api/connections.md#psycopg.Connection.connect) and [`cursor()`](../api/connections.md#psycopg.Connection.cursor)
...
Note that the [`AsyncConnection.cursor()`](../api/connections.md#psycopg.AsyncConnection.cursor) function is not an `async` function
...
Python [CancelledError](https://docs.python.org/3/library/asyncio-task.html#task-cancellation).
...
then consider calling [`Connection.cancel()`](../api/connections.md#psycopg.Connection.cancel) as an alternative to cancelling
the task.
...
Psycopg 3 supports [gevent](https://www.gevent.org/) out of the box. If the
[`select`](https://docs.python.org/3/library/select.html#module-select) module is found patched by functions such as
[gevent.monkey.patch_select()](http://www.gevent.org/api/gevent.monkey.html#gevent.monkey.patch_select) or [patch_all()](http://www.gevent.org/api/gevent.monkey.html#gevent.monkey.patch_all), psycopg will behave in a
collaborative way.
...
<a id="async-messages"></a>

## Server messages

PostgreSQL can send, together with the query results, [informative messages](https://www.postgresql.org/docs/current/runtime-config-logging.html#RUNTIME-CONFIG-SEVERITY-LEVELS)
...
Messages can be also sent by the [PL/pgSQL ‘RAISE’ statement](https://www.postgresql.org/docs/current/plpgsql-errors-and-messages.html) (at a level
lower than EXCEPTION, otherwise the appropriate [`DatabaseError`](../api/errors.md#psycopg.DatabaseError) will be
raised). The level of the messages received can be controlled using the
[client_min_messages](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-CLIENT-MIN-MESSAGES) setting.

By default, the messages received are ignored. If you want to process them on
the client you can use the [`Connection.add_notice_handler()`](../api/connections.md#psycopg.Connection.add_notice_handler) function to
register a function that will be invoked whenever a message is received. The
message is passed to the callback as a [`Diagnostic`](../api/errors.md#psycopg.errors.Diagnostic) instance,
containing all the information passed by the server, such as the message text
and the severity. The object is the same found on the [`diag`](../api/errors.md#psycopg.Error.diag)
...
facilities offered by PostgreSQL commands [`LISTEN`](https://www.postgresql.org/docs/current/sql-listen.html) and [`NOTIFY`](https://www.postgresql.org/docs/current/sql-notify.html). Please
refer to the PostgreSQL documentation for examples about how to use this form
of communication.

Because of the way transactions interact with notifications (see [`NOTIFY`](https://www.postgresql.org/docs/current/sql-notify.html)
documentation), you should keep the connection in [`autocommit`](../api/connections.md#psycopg.Connection.autocommit)
mode if you wish to receive or send notifications in a timely manner.

#### NOTE
You don’t need an [`AsyncConnection`](../api/connections.md#psycopg.AsyncConnection) to handle notifications: a normal
blocking [`Connection`](../api/connections.md#psycopg.Connection) is perfectly valid.

Notifications are received as instances of the [`Notify`](../api/objects.md#psycopg.Notify) object. You can
receive notifications using either the [notifies generator](#notifies-generator) or a [notifies handler](#notifies-handlers).
...
<a id="notifies-generator"></a>

### Notifies generator

If you are reserving a connection only to receive notifications, the simplest
way to receive them is to consume the [`Connection.notifies()`](../api/connections.md#psycopg.Connection.notifies) generator. The
...
- Since [Psycopg 3.2.4](../news.md#psycopg-3-2-4) such notifications are captured and yielded
  by the generator once it is started. However this introduced a leak
  (ticket [#1091](https://github.com/psycopg/psycopg/issues/1091)) and, if the generator is not regularly used, some
  memory is allocated indefinitely.
- Since [Psycopg 3.2.10](../news.md#psycopg-3-2-10) if a handler is registered then
...
<a id="notifies-handlers"></a>

### Notifies handlers

Alternatively, you can use [`add_notify_handler()`](../api/connections.md#psycopg.Connection.add_notify_handler) to register a
...
In a normal (non asyncio) program you can use the [`selectors`](https://docs.python.org/3/library/selectors.html#module-selectors) module. Because
the `Connection` implements a [`fileno()`](../api/connections.md#psycopg.Connection.fileno) method you can just
...
In an [`asyncio`](https://docs.python.org/3/library/asyncio.html#module-asyncio) program you can dedicate a [`Task`](https://docs.python.org/3/library/asyncio-task.html#asyncio.Task) instead and do
something similar using [`add_reader`](https://docs.python.org/3/library/asyncio-eventloop.html#asyncio.loop.add_reader):
