<a id="connection-pools"></a>

# Connection pools

A [connection pool](https://en.wikipedia.org/wiki/Connection_pool) is an object managing a set of connections and allowing
...
behaviour. Please refer to the [`ConnectionPool`](../api/pool.md#psycopg_pool.ConnectionPool) object API for details about
the pool operations.

#### NOTE
The connection pool objects are distributed in a package separate
from the main [`psycopg`](../api/module.md#module-psycopg) package: use `pip install "psycopg[pool]"` or `pip
install psycopg_pool` to make the [`psycopg_pool`](../api/pool.md#module-psycopg_pool) package available. See
[Installing the connection pool](../basic/install.md#pool-installation).

## Basic connection pool usage

A [`ConnectionPool`](../api/pool.md#psycopg_pool.ConnectionPool) object can be used to request connections from multiple
...
[`connection()`](../api/pool.md#psycopg_pool.ConnectionPool.connection) method, and use it as a context manager too:
...
The `connection()` context behaves like the [`Connection`](../api/connections.md#psycopg.Connection) object
...
exited with an exception. See [Transaction contexts](../basic/transactions.md#transaction-context) for details.
...
[`AsyncConnectionPool`](../api/pool.md#psycopg_pool.AsyncConnectionPool) instead and use the `async` and `await` keywords with
...
block until failing with a [`PoolTimeout`](../api/pool.md#psycopg_pool.PoolTimeout).

If you want to make sure early in the application lifetime that the
environment is well configured, you can use the [`wait()`](../api/pool.md#psycopg_pool.ConnectionPool.wait) method
...
- the worker creates a connection according to the parameters `conninfo`,
  `kwargs`, and `connection_class` passed to [`ConnectionPool`](../api/pool.md#psycopg_pool.ConnectionPool) constructor,
  calling something similar to `*connection_class*(*conninfo*,
  ***kwargs*)`;
...
[`connection()`](../api/pool.md#psycopg_pool.ConnectionPool.connection) context):
...
context stared by [`connection()`](../api/pool.md#psycopg_pool.ConnectionPool.connection)):
...
call the [`open()`](../api/pool.md#psycopg_pool.ConnectionPool.open) method.

If you are not using the pool as context manager, you are advised to call the
[`close()`](../api/pool.md#psycopg_pool.ConnectionPool.close) method on program exit: on some Python versions this
might cause a program exit delay. How to ensure it depends on the way you are
writing your program. One simple method is to use the [`atexit`](https://docs.python.org/3/library/atexit.html#module-atexit) module:
...
can use a [lifespan](https://fastapi.tiangolo.com/advanced/events/#lifespan) function:
...
in the future. When using [`AsyncConnectionPool`](../api/pool.md#psycopg_pool.AsyncConnectionPool) you should call `await
...
the [`ConnectionPool`](../api/pool.md#psycopg_pool.ConnectionPool) API is different from the normal [`connect()`](../api/module.md#psycopg.connect)
...
The `psycopg_pool` 3.1 package introduces the [`NullConnectionPool`](../api/pool.md#psycopg_pool.NullConnectionPool) class.
...
analysis](https://github.com/brettwooldridge/HikariCP/blob/dev/documents/Welcome-To-The-Jungle.md), for instance).
...
take a look at [this analysis](https://github.com/brettwooldridge/HikariCP/wiki/About-Pool-Sizing) for some ideas.

Something useful you can do is probably to use the
[`get_stats()`](../api/pool.md#psycopg_pool.ConnectionPool.get_stats) method and monitor the behaviour of your program
to tune the configuration parameters. The size of the pool can also be changed
at runtime using the [`resize()`](../api/pool.md#psycopg_pool.ConnectionPool.resize) method.
...
[idle_session_timeout](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-IDLE-SESSION-TIMEOUT) killing connections that haven’t been used for some
...
[`ConnectionPool.check_connection`](../api/pool.md#psycopg_pool.ConnectionPool.check_connection), which can be used as:
...
The pool uses the [`logging`](https://docs.python.org/3/library/logging.html#module-logging) module to log some key operations to the
...
[`get_stats()`](../api/pool.md#psycopg_pool.ConnectionPool.get_stats) or [`pop_stats()`](../api/pool.md#psycopg_pool.ConnectionPool.pop_stats). Both methods
return the same values, but the latter reset the counters after its use. The
values can be sent to a monitoring system such as [Graphite](https://graphiteapp.org/) or [Prometheus](https://prometheus.io/).
...
| Metric               | Meaning                                                                                                                               |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| `pool_min`           | Current value for [`min_size`](../api/pool.md#psycopg_pool.ConnectionPool.min_size)                                                   |
| `pool_max`           | Current value for [`max_size`](../api/pool.md#psycopg_pool.ConnectionPool.max_size)                                                   |
| `pool_size`          | Number of connections currently managed by the pool<br/>(in the pool, given to clients, being prepared)                               |
| `pool_available`     | Number of connections currently idle in the pool                                                                                      |
| `requests_waiting`   | Number of requests currently waiting in a queue to<br/>receive a connection                                                           |
| `usage_ms`           | Total usage time of the connections outside the pool                                                                                  |
| `requests_num`       | Number of connections requested to the pool                                                                                           |
| `requests_queued`    | Number of requests queued because a connection wasn’t<br/>immediately available in the pool                                           |
| `requests_wait_ms`   | Total time in the queue for the clients waiting                                                                                       |
| `requests_errors`    | Number of connection requests resulting in an error<br/>(timeouts, queue full…)                                                       |
| `returns_bad`        | Number of connections returned to the pool in a bad<br/>state                                                                         |
| `connections_num`    | Number of connection attempts made by the pool to the<br/>server                                                                      |
| `connections_ms`     | Total time spent to establish connections with the<br/>server                                                                         |
| `connections_errors` | Number of failed connection attempts                                                                                                  |
| `connections_lost`   | Number of connections lost identified by<br/>[`check()`](../api/pool.md#psycopg_pool.ConnectionPool.check) or by the `check` callback |

<a id="pool-sqlalchemy"></a>
...
If you want to use [SQLAlchemy](https://sqlalchemy.org) with psycopg’s connection pool you can use the
SQLAlchemy’s [NullPool](https://docs.sqlalchemy.org/en/20/core/pooling.html#sqlalchemy.pool.NullPool) feature, using the pool’s [`getconn`](../api/pool.md#psycopg_pool.ConnectionPool.getconn)
as `creator` function. However, SQLAlchemy expects that calling
[`close()`](../api/connections.md#psycopg.Connection.close) on the connection will return the connection to
...
[`Connection`](../api/connections.md#psycopg.Connection) or [`AsyncConnection`](../api/connections.md#psycopg.AsyncConnection) class, overriding the
