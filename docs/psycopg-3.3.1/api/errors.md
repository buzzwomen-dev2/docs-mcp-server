...
```default
`Exception`
|__ [`Warning`](#psycopg.Warning)
|__ [`Error`](#psycopg.Error)
    |__ [`InterfaceError`](#psycopg.InterfaceError)
    |__ [`DatabaseError`](#psycopg.DatabaseError)
        |__ [`DataError`](#psycopg.DataError)
        |__ [`OperationalError`](#psycopg.OperationalError)
        |__ [`IntegrityError`](#psycopg.IntegrityError)
        |__ [`InternalError`](#psycopg.InternalError)
        |__ [`ProgrammingError`](#psycopg.ProgrammingError)
        |__ [`NotSupportedError`](#psycopg.NotSupportedError)
```

These classes are exposed both by this module and the root [`psycopg`](module.md#module-psycopg) module.
...
#### sqlstate *: [str](https://docs.python.org/3/library/stdtypes.html#str) | [None](https://docs.python.org/3/library/constants.html#None)* *= None*

The code of the error, if received from the server.

This attribute is also available as class attribute on the
[SQLSTATE exceptions](#sqlstate-exceptions) classes.

#### pgconn *: [pq.PGconn](pq.md#psycopg.pq.PGconn) | [None](https://docs.python.org/3/library/constants.html#None)*

The connection object, if the error was raised from a connection attempt.

It has been closed and will be in [`BAD`](pq.md#psycopg.pq.ConnStatus.BAD) state;
however it might be useful to verify precisely what went wrong, for
instance checking the [`needs_password`](pq.md#psycopg.pq.PGconn.needs_password) and
[`used_password`](pq.md#psycopg.pq.PGconn.used_password) attributes.
...
#### pgresult *: [pq.PGresult](pq.md#psycopg.pq.PGresult) | [None](https://docs.python.org/3/library/constants.html#None)*
...
Exception raised on timeout of the [`connect()`](connections.md#psycopg.Connection.connect) method.

The error is raised if the `connect_timeout` is specified and a
connection is not obtained in useful time.

Subclass of [`OperationalError`](#psycopg.OperationalError).

### *exception* psycopg.errors.CancellationTimeout

Exception raised on timeout of connection’s
[`cancel_safe()`](connections.md#psycopg.Connection.cancel_safe) method.
...
The object is available as the [`Error`](#psycopg.Error).[`diag`](#psycopg.Error.diag)
attribute and is passed to the callback functions registered with
[`add_notice_handler()`](connections.md#psycopg.Connection.add_notice_handler).

All the information available from the `[PQresultErrorField()](https://www.postgresql.org/docs/18/libpq-exec.html#LIBPQ-PQRESULTERRORFIELD)` function
...
called SQLSTATE (available in the [`sqlstate`](#psycopg.errors.Diagnostic.sqlstate) attribute of the
error’s [`diag`](#psycopg.Error.diag) attribute).
...
the documentation](https://www.postgresql.org/docs/current/errcodes-appendix.html#ERRCODES-TABLE), converted to CamelCase: e.g. the error 22012,
...
name clashes: please refer to the [table below](#exceptions-list) for all
the classes defined.

Every exception class is a subclass of one of the [standard DB-API
exception](#dbapi-exceptions), thus exposing the [`Error`](#psycopg.Error) interface.

#### Versionchanged
Changed in version 3.1.4: Added exceptions introduced in PostgreSQL 15.

### psycopg.errors.lookup(sqlstate: [str](https://docs.python.org/3/library/stdtypes.html#str)) → [type](https://docs.python.org/3/library/functions.html#type)[[Error](#psycopg.Error)]

Lookup an error code or [constant name](https://www.postgresql.org/docs/current/errcodes-appendix.html#ERRCODES-TABLE) and return its exception class.
...
#### Versionadded
Added in version 3.1.4: Exception `SqlJsonItemCannotBeCastToTargetType`, introduced in PostgreSQL 15.

#### Versionadded
Added in version 3.2.3: Exception `TransactionTimeout`, introduced in PostgreSQL 17.

#### Versionadded
Added in version 3.2.10: Exception `InvalidArgumentForXquery`, `FileNameTooLong`, introduced in
PostgreSQL 18.
