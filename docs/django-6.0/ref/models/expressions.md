...
[`save()`](instances.md#django.db.models.Model.save) on backends that support it without incurring a subsequent
...
database on [`save()`](instances.md#django.db.models.Model.save) which resulted in them being evaluated and
...
[output_field](#output-field) for this case, but since `F()` does not,
you will need to wrap the expression with [`ExpressionWrapper`](#django.db.models.ExpressionWrapper):
...
See [Database Functions](database-functions.md) for a list of built-in database
functions.
...
not contain untrusted user input](#avoiding-sql-injection-in-query-expressions) as these values are
...
The `filter` argument takes a [Q object](querysets.md#q-objects) that’s
...
[`order_by()`](querysets.md#django.db.models.query.QuerySet.order_by) function, accepting a field name (with an optional
...
[`JSONField`](fields.md#django.db.models.JSONField).

See [Storing and querying for None](../../topics/db/queries.md#storing-and-querying-for-none) for usage examples.
...
example, to support [composite primary keys](../../topics/composite-primary-key.md#cpk-and-database-functions). Defaults to `False`.
...
<a id="writing-your-own-query-expressions"></a>
...
should live in an [`AppConfig.ready()`](../applications.md#django.apps.AppConfig.ready)
method.
