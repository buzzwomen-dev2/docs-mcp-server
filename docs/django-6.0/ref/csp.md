...
<a id="module-django.middleware.csp"></a>
...
loaded. It plays an important role in a comprehensive [security strategy](../topics/security.md#security-csp).

For configuration instructions in a Django project, see the [Using CSP](../howto/csp.md#csp-config) documentation. For an HTTP guide about CSP, see the [MDN Guide on
...
The [Content-Security-Policy specification](https://www.w3.org/TR/CSP3/)
...
When the [`ContentSecurityPolicyMiddleware`](middleware.md#django.middleware.csp.ContentSecurityPolicyMiddleware) is
enabled, Django automatically builds and attaches the appropriate headers to
each response based on the configured [settings](#csp-settings), unless
...
The [`ContentSecurityPolicyMiddleware`](middleware.md#django.middleware.csp.ContentSecurityPolicyMiddleware) is
configured using the following settings:

* [`SECURE_CSP`](settings.md#std-setting-SECURE_CSP): defines the **enforced Content Security Policy**.
* [`SECURE_CSP_REPORT_ONLY`](settings.md#std-setting-SECURE_CSP_REPORT_ONLY): defines a **report-only Content Security
  Policy**.
...
reports programmatically, the policy must include a [reporting directive](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy#reporting_directives)
...
[`SECURE_CSP_REPORT_ONLY`](settings.md#std-setting-SECURE_CSP_REPORT_ONLY) settings, but reports will only be issued by
...
They are available through the [`CSP`](#django.utils.csp.CSP) enum, and using
...
<a id="module-django.utils.csp"></a>
...
[`ContentSecurityPolicyMiddleware`](middleware.md#django.middleware.csp.ContentSecurityPolicyMiddleware) with a
secure, random nonce that is generated for each request. See detailed
explanation in [Nonce usage](#csp-nonce).
...
constants defined in [`CSP`](#django.utils.csp.CSP).
...
using directives in the same format as the [`SECURE_CSP`](settings.md#std-setting-SECURE_CSP) setting.
...
[`SECURE_CSP_REPORT_ONLY`](settings.md#std-setting-SECURE_CSP_REPORT_ONLY) setting.

Like [`csp_override()`](#django.views.decorators.csp.csp_override), the `config` argument must be a mapping with
...
[guide for decorating class-based views](../topics/class-based-views/intro.md#id1).
...
[`NONCE`](#django.utils.csp.CSP.NONCE) in the relevant directive(s) of your
[CSP settings](#csp-settings), such as `script-src` or `style-src`.
When present, the
[`ContentSecurityPolicyMiddleware`](middleware.md#django.middleware.csp.ContentSecurityPolicyMiddleware)
...
[`csp()`](templates/api.md#django.template.context_processors.csp) context processor needs to be
...
[`NONCE`](#django.utils.csp.CSP.NONCE), the HTML will include a nonce attribute,
