...
request.method == "POST"` (see [`HttpRequest.method`](#django.http.HttpRequest.method)).
...
[`UploadedFile`](files/uploads.md#django.core.files.uploadedfile.UploadedFile).
...
[**RFC 9110 Section 12.5.1**](https://datatracker.ietf.org/doc/html/rfc9110.html#section-12.5.1).)
...
[Content negotiation example](../topics/class-based-views/generic-editing.md#content-negotiation-example) for an example of returning
...
[`cache middleware`](middleware.md#module-django.middleware.cache), you should decorate
the view with [`vary_on_headers('Accept')`](../topics/http/decorators.md#django.views.decorators.vary.vary_on_headers) so that the responses
are properly cached.
...
See [Content negotiation example](../topics/class-based-views/generic-editing.md#content-negotiation-example) for an example of using
...
[`POST`](#django.http.HttpRequest.POST) attributes are instances of
...
Like [`dict.setdefault()`](https://docs.python.org/3/library/stdtypes.html#dict.setdefault), except it uses [`__setitem__()`](#django.http.QueryDict.__setitem__)
internally.
...
[`__getitem__()`](#django.http.QueryDict.__getitem__) and returns an iterator object instead of a view
object. For example:
...
[`django.utils.cache`](utils.md#module-django.utils.cache), since these fields can have multiple,
...
A string representation of [`HttpResponse.content`](#django.http.HttpResponse.content), decoded using the
response’s [`HttpResponse.charset`](#django.http.HttpResponse.charset) (defaulting to `UTF-8` if empty).
...
See [`HttpResponse`](#django.http.HttpResponse) for other optional constructor arguments.
