...
* `database`: Checks database-related configuration issues. Database checks
  ...
  most regular checks do. They are only run by the [`migrate`](django-admin.md#django-admin-migrate) command
  or if you specify configured database aliases using the `--database` option
  when calling the [`check`](django-admin.md#django-admin-check) command.
...
* **security.W014**: You have
  [`django.contrib.sessions.middleware.SessionMiddleware`](middleware.md#django.contrib.sessions.middleware.SessionMiddleware) in your
  [`MIDDLEWARE`](settings.md#std-setting-MIDDLEWARE), but you have not set
  [`SESSION_COOKIE_HTTPONLY`](settings.md#std-setting-SESSION_COOKIE_HTTPONLY) to `True`. Using an `HttpOnly` session
  cookie makes it more difficult for cross-site scripting attacks to hijack
  user sessions.
