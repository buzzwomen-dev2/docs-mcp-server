...
service. Given that, a [task backend](#configuring-a-task-backend) capable
...
<a id="configuring-a-task-backend"></a>
...
your application. Django comes with [built-in backends](../ref/tasks.md#task-available-backends), but these are for development and testing only.
...
runs queued work. Relevant options are listed in the [Community Ecosystem](https://www.djangoproject.com/community/ecosystem/) page.

Task backends are configured using the [`TASKS`](../ref/settings.md#std-setting-TASKS) setting in your
settings file. Whilst most applications will only need a single backend,
multiple are supported.

<a id="immediate-task-backend"></a>

### Immediate execution

This is the default backend if another is not specified in your settings file.
The [`ImmediateBackend`](../ref/tasks.md#django.tasks.backends.immediate.ImmediateBackend) runs enqueued Tasks immediately, rather than in
...
To use it, set [`BACKEND`](../ref/settings.md#std-setting-TASKS-BACKEND) to
...
The [`ImmediateBackend`](../ref/tasks.md#django.tasks.backends.immediate.ImmediateBackend) may also be useful in tests, to bypass the need
to run a real background worker in your tests.

<a id="dummy-task-backend"></a>

### Dummy backend

The [`DummyBackend`](../ref/tasks.md#django.tasks.backends.dummy.DummyBackend) doesn’t execute enqueued Tasks at all, instead
storing results for later use. Task results will forever remain in the
[`READY`](../ref/tasks.md#django.tasks.TaskResultStatus.READY) state.
...
[`results`](../ref/tasks.md#django.tasks.backends.dummy.DummyBackend.results) attribute:
...
[`clear()`](../ref/tasks.md#django.tasks.backends.dummy.DummyBackend.clear) method:
...
[`BACKEND`](../ref/settings.md#std-setting-TASKS-BACKEND) of the [`TASKS`](../ref/settings.md#std-setting-TASKS) setting, like so:
...
[`BaseTaskBackend`](../ref/tasks.md#django.tasks.backends.base.BaseTaskBackend). At a minimum, it must
implement [`BaseTaskBackend.enqueue()`](../ref/tasks.md#django.tasks.backends.base.BaseTaskBackend.enqueue). If you’re building your own
backend, you can use the built-in Task backends as reference implementations.
You’ll find the code in the [django/tasks/backends/](https://github.com/django/django/blob/main/django/tasks/backends/) directory of the
Django source.
...
[`BaseTaskBackend`](../ref/tasks.md#django.tasks.backends.base.BaseTaskBackend) has async variants of all
...
<a id="defining-tasks"></a>

## Defining Tasks

Tasks are defined using the [`django.tasks.task()`](../ref/tasks.md#django.tasks.task) decorator on a
...
The return value of the decorator is a [`Task`](../ref/tasks.md#django.tasks.Task) instance.

[`Task`](../ref/tasks.md#django.tasks.Task) attributes can be customized via the `@task`
decorator arguments:
...
[`TaskContext`](../ref/tasks.md#django.tasks.TaskContext).
...
can be created with the [`using()`](../ref/tasks.md#django.tasks.Task.using) method, leaving the
...
<a id="enqueueing-tasks"></a>

## Enqueueing Tasks

To add the Task to the queue store, so it will be executed, call the
[`enqueue()`](../ref/tasks.md#django.tasks.Task.enqueue) method on it. If the Task takes arguments,
...
This returns a [`TaskResult`](../ref/tasks.md#django.tasks.TaskResult), which can be used to retrieve
the result of the Task once it has finished executing.

To enqueue Tasks in an `async` context, [`aenqueue()`](../ref/tasks.md#django.tasks.Task.aenqueue)
is available as an `async` variant of [`enqueue()`](../ref/tasks.md#django.tasks.Task.enqueue).
...
Arguments must also be able to round-trip through a [`json.dumps()`](https://docs.python.org/3/library/json.html#json.dumps)/
[`json.loads()`](https://docs.python.org/3/library/json.html#json.loads) cycle without changing type. For example, consider this
Task:
...
committed to the database, use [`transaction.on_commit()`](db/transactions.md#django.db.transaction.on_commit), binding all arguments to
[`enqueue()`](../ref/tasks.md#django.tasks.Task.enqueue) via [`functools.partial()`](https://docs.python.org/3/library/functools.html#functools.partial):
...
When enqueueing a `Task`, you receive a [`TaskResult`](../ref/tasks.md#django.tasks.TaskResult),
however it’s likely useful to retrieve the result from somewhere else (for
example another request or another Task).

Each `TaskResult` has a unique [`id`](../ref/tasks.md#django.tasks.TaskResult.id), which can
be used to identify and retrieve the result once the code which enqueued the
Task has finished.

The [`get_result()`](../ref/tasks.md#django.tasks.Task.get_result) method can retrieve a result based on
its `id`:
...
use the [`get_result()`](../ref/tasks.md#django.tasks.Task.get_result) method on the backend:
...
[`aget_result()`](../ref/tasks.md#django.tasks.Task.aget_result) is available as an `async` variant of
[`get_result()`](../ref/tasks.md#django.tasks.Task.get_result) on both the backend and `Task`.
...
retrieved. If the Task finishes after [`get_result()`](../ref/tasks.md#django.tasks.Task.get_result) is
called, it will not update.

To refresh the values, call the [`django.tasks.TaskResult.refresh()`](../ref/tasks.md#django.tasks.TaskResult.refresh)
method:
...
<a id="task-return-values"></a>

### Return values

If your Task function returns something, it can be retrieved from the
[`django.tasks.TaskResult.return_value`](../ref/tasks.md#django.tasks.TaskResult.return_value) attribute:
...
If the Task has not finished executing, or has failed, [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError) is
raised.
...
[`django.tasks.TaskResult.errors`](../ref/tasks.md#django.tasks.TaskResult.errors) list.

Each entry in `errors` is a [`TaskError`](../ref/tasks.md#django.tasks.TaskError) containing
