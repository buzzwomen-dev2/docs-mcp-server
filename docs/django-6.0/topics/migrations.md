...
To remove old references, you can [squash migrations](#migration-squashing) or, if there aren’t many references, copy them into the
migration files.
...
[`RunPython`](../ref/migration-operations.md#django.db.migrations.operations.RunPython) expects a callable as its
argument which takes two arguments - the first is an [app registry](../ref/applications.md) that has the historical versions of all your models
...
- [`zoneinfo.ZoneInfo`](https://docs.python.org/3/library/zoneinfo.html#zoneinfo.ZoneInfo) instances
...
- Anything with a custom `deconstruct()` method
  ([see below](#custom-deconstruct-method))

#### Versionchanged
Serialization support for [`zoneinfo.ZoneInfo`](https://docs.python.org/3/library/zoneinfo.html#zoneinfo.ZoneInfo) instances was added.
