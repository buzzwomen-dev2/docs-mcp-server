...
When using the [`DjangoTemplates`](#django.template.backends.django.DjangoTemplates)
backend, a specific fragment from a template can be loaded by name. This
fragment should have been previously defined using the [`partialdef`](../ref/templates/builtins.md#std-templatetag-partialdef)
template tag:
...
See [Template partials](../ref/templates/language.md#template-partials) for more information about defining and using
template fragments.

#### Versionchanged
Support for loading template partials when using the
[`DjangoTemplates`](#django.template.backends.django.DjangoTemplates) backend was
added.
...
[`HttpRequest`](../ref/request-response.md#django.http.HttpRequest). Then the engine must make it, as well as
