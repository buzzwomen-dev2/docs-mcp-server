...
Use [`send_mail()`](#django.core.mail.send_mail) for straightforward email sending. For example, to send a
plain text message:
...
[`EmailMessage`](#django.core.mail.EmailMessage) or [`EmailMultiAlternatives`](#django.core.mail.EmailMultiAlternatives). For example, to send
...
same functions as in [`send_mail()`](#django.core.mail.send_mail). They must be given as keyword arguments
if used.

Each separate element of `datatuple` results in a separate email message.
As in [`send_mail()`](#django.core.mail.send_mail), recipients in the same `recipient_list` will all see
...
The main difference between [`send_mass_mail()`](#django.core.mail.send_mass_mail) and [`send_mail()`](#django.core.mail.send_mail) is
that [`send_mail()`](#django.core.mail.send_mail) opens a connection to the mail server each time it’s
executed, while [`send_mass_mail()`](#django.core.mail.send_mass_mail) uses a single connection for all of its
messages. This makes [`send_mass_mail()`](#django.core.mail.send_mass_mail) slightly more efficient.
...
the email function (e.g. [`send_mail()`](#django.core.mail.send_mail)) will raise [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError) and,
...
Django’s [`send_mail()`](#django.core.mail.send_mail) and [`send_mass_mail()`](#django.core.mail.send_mass_mail) functions are actually
thin wrappers that make use of the [`EmailMessage`](#django.core.mail.EmailMessage) class.

Not all features of the [`EmailMessage`](#django.core.mail.EmailMessage) class are available through the
[`send_mail()`](#django.core.mail.send_mail) and related wrapper functions. If you wish to use advanced
features, such as BCC’ed recipients, file attachments, or multi-part email,
you’ll need to create [`EmailMessage`](#django.core.mail.EmailMessage) instances directly.

#### NOTE
This is a design feature. [`send_mail()`](#django.core.mail.send_mail) and related functions were
...
[`EmailMessage`](#django.core.mail.EmailMessage) is responsible for creating the email message itself. The
[email backend](#topic-email-backends) is then responsible for sending the
email.

For convenience, [`EmailMessage`](#django.core.mail.EmailMessage) provides a [`send()`](#django.core.mail.EmailMessage.send)
...
to calling the [`send()`](#django.core.mail.EmailMessage.send) method.
...
* `attachments`: A list of attachments to put on the message. Each can
  be an instance of [`MIMEPart`](https://docs.python.org/3/library/email.message.html#email.message.MIMEPart) or
  [`EmailAttachment`](#django.core.mail.EmailAttachment), or a tuple with attributes
  `(filename, content, mimetype)`.

  #### Versionchanged
  Support for [`MIMEPart`](https://docs.python.org/3/library/email.message.html#email.message.MIMEPart) objects in the
  `attachments` list was added.

  #### Deprecated
  Deprecated since version 6.0: Support for Python’s legacy [`MIMEBase`](https://docs.python.org/3/library/email.mime.html#email.mime.base.MIMEBase)
  objects in `attachments` is deprecated. Use
  [`MIMEPart`](https://docs.python.org/3/library/email.message.html#email.message.MIMEPart) instead.
* `headers`: A dictionary of extra headers to put on the message. The
  keys are the header name, values are the header values. It’s up to the
  caller to ensure header names and values are in the correct format for
  an email message. The corresponding attribute is `extra_headers`.
* `connection`: An [email backend](#topic-email-backends) instance.
  Use this parameter if you are sending the `EmailMessage` via
  [`send()`](#django.core.mail.EmailMessage.send) and you want to use the same connection for multiple
  messages. If omitted, a new connection is created when [`send()`](#django.core.mail.EmailMessage.send) is
  called. This parameter is ignored when using
  [send_messages()](#topics-sending-multiple-emails).
...
Constructs and returns a Python [`email.message.EmailMessage`](https://docs.python.org/3/library/email.message.html#email.message.EmailMessage)
...
an [`email.policy.Policy`](https://docs.python.org/3/library/email.policy.html#module-email.policy) object. Defaults to
[`email.policy.default`](https://docs.python.org/3/library/email.policy.html#email.policy.default). In certain cases you may want to use
[`SMTP`](https://docs.python.org/3/library/email.policy.html#email.policy.SMTP), [`SMTPUTF8`](https://docs.python.org/3/library/email.policy.html#email.policy.SMTPUTF8) or a custom
policy. For example,
[`django.core.mail.backends.smtp.EmailBackend`](#django.core.mail.backends.smtp.EmailBackend) uses the
[`SMTP`](https://docs.python.org/3/library/email.policy.html#email.policy.SMTP) policy to ensure `\r\n` line endings as
required by the SMTP protocol.

If you ever need to extend Django’s [`EmailMessage`](#django.core.mail.EmailMessage) class,
...
updated to an instance of [`EmailMessage`](https://docs.python.org/3/library/email.message.html#email.message.EmailMessage).
...
* You can pass it three arguments: `filename`, `content` and
  ...
  `content` can be a [`django.core.mail.EmailMessage`](#django.core.mail.EmailMessage) or
  Python’s [`email.message.EmailMessage`](https://docs.python.org/3/library/email.message.html#email.message.EmailMessage) or
...
* Or for attachments requiring additional headers or parameters, you
  ...
  [`MIMEPart`](https://docs.python.org/3/library/email.message.html#email.message.MIMEPart) object. This will be attached
  ...
  Python’s [`email.contentmanager.set_content()`](https://docs.python.org/3/library/email.contentmanager.html#email.contentmanager.set_content) documentation
  describes the supported arguments for `MIMEPart.set_content()`.

  #### Versionchanged
  Support for [`MIMEPart`](https://docs.python.org/3/library/email.message.html#email.message.MIMEPart) attachments was
  added.

  #### Deprecated
  Deprecated since version 6.0: Support for [`email.mime.base.MIMEBase`](https://docs.python.org/3/library/email.mime.html#email.mime.base.MIMEBase) attachments is
  deprecated. Use [`MIMEPart`](https://docs.python.org/3/library/email.message.html#email.message.MIMEPart) instead.
...
as in [`attach()`](#django.core.mail.EmailMessage.attach).
...
A list of [`EmailAlternative`](#django.core.mail.EmailAlternative) named tuples.
...
By default, the MIME type of the `body` parameter in an [`EmailMessage`](#django.core.mail.EmailMessage)
...
attribute on the [`EmailMessage`](#django.core.mail.EmailMessage) class to change the main content type.
...
* `send_messages(email_messages)` sends a list of [`EmailMessage`](#django.core.mail.EmailMessage)
...
written is either taken from the [`EMAIL_FILE_PATH`](../ref/settings.md#std-setting-EMAIL_FILE_PATH) setting or from the
`file_path` keyword when creating a connection with [`get_connection()`](#django.core.mail.get_connection).
...
message is sent. It’s a list with an [`EmailMessage`](#django.core.mail.EmailMessage) instance for each
...
list of [`EmailMessage`](#django.core.mail.EmailMessage) instances and returns the number of successfully
...
a list of [`EmailMessage`](#django.core.mail.EmailMessage) (or subclass) instances, and sends them all
using that single connection. As a consequence, any [`connection`](#django.core.mail.EmailMessage) set on an individual message is ignored.

For example, if you have a function called `get_notification_email()` that
returns a list of [`EmailMessage`](#django.core.mail.EmailMessage) objects representing some periodic
