...
* Fixed a crash when using `select_related` against a `ForeignObject`
  originating from a model with a `CompositePrimaryKey` ([#36373](https://code.djangoproject.com/ticket/36373)).
* Fixed a bug in Django 5.2 where subqueries using `"pk"` to reference models
  ...
  or too few columns were selected ([#36392](https://code.djangoproject.com/ticket/36392)).
* Fixed a regression in Django 5.2 that caused a crash when no arguments were
  passed into `QuerySet.union()` ([#36388](https://code.djangoproject.com/ticket/36388)).
* Fixed a regression in Django 5.2 where subclasses of `RemoteUserMiddleware`
  ...
  ([#36390](https://code.djangoproject.com/ticket/36390)).
* Fixed a regression in Django 5.2 that caused a crash when using `OuterRef`
  in the `filter` argument of an `Aggregate` expression ([#36404](https://code.djangoproject.com/ticket/36404)).
* Fixed a regression in Django 5.2 that caused a crash when using `OuterRef`
  in PostgreSQL aggregate functions `ArrayAgg`, `StringAgg`, and
  `JSONBAgg` ([#36405](https://code.djangoproject.com/ticket/36405)).
* Fixed a regression in Django 5.2 where admin’s `filter_horizontal` buttons
  ...
  pressing the Enter key ([#36423](https://code.djangoproject.com/ticket/36423)).
* Fixed a bug in Django 5.2 where calling `QuerySet.in_bulk()` with an
  ...
  observe database parameter limits ([#36416](https://code.djangoproject.com/ticket/36416)).
* Fixed a bug in Django 5.2 where [`HttpRequest.get_preferred_type()`](../ref/request-response.md#django.http.HttpRequest.get_preferred_type) did not account for media type
  parameters in `Accept` headers, reducing specificity in content negotiation
  ([#36411](https://code.djangoproject.com/ticket/36411)).
* Fixed a regression in Django 5.2 that caused a crash when using
  ...
  queryset for a subclass of the foreign target ([#36432](https://code.djangoproject.com/ticket/36432)).
